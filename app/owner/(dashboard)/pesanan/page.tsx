...
// Template pesan WA otomatis per status pesanan. Placeholder {nama}, {no_transaksi},
// dan {link_lacak} disisipkan otomatis saat tombol WA ditekan.
const STATUS_WA_TEMPLATE: Record<StatusTransaksi, string> = {
  pending: 'Halo {nama}, terima kasih sudah pesan di BiteSeyo! Pesanan kamu dengan no. transaksi {no_transaksi} sudah kami terima dan sedang menunggu konfirmasi pembayaran ya. Cek status pesanan kapan saja di {link_lacak}',
  dikonfirmasi: 'Halo {nama}, pembayaran untuk pesanan {no_transaksi} sudah kami konfirmasi. Pesanan kamu segera kami proses ya, terima kasih! Cek status di {link_lacak}',
  diproses: 'Halo {nama}, pesanan kamu ({no_transaksi}) sedang kami siapkan sekarang. Ditunggu ya, kami kabari lagi kalau sudah siap dikirim/diambil. Cek status di {link_lacak}',
  dikirim: 'Halo {nama}, pesanan kamu ({no_transaksi}) sudah dalam perjalanan/siap diambil. Mohon ditunggu ya, terima kasih sudah pesan di BiteSeyo! Cek status di {link_lacak}',
  selesai: 'Halo {nama}, pesanan kamu ({no_transaksi}) sudah selesai. Terima kasih banyak sudah belanja di BiteSeyo, semoga suka! Ditunggu pesanan berikutnya ya 😊',
  dibatalkan: 'Halo {nama}, mohon maaf pesanan kamu ({no_transaksi}) kami batalkan. Kalau ada pertanyaan atau ingin pesan ulang, silakan balas chat ini ya.',
};

function buildWaLink(t: Transaksi): string | null {
  const nomor = formatNoWaLink(t.pelanggan?.no_wa);
  if (!nomor) return null;
  const linkLacak = typeof window !== 'undefined' ? `${window.location.origin}/lacak/${t.kode_lacak}` : `/lacak/${t.kode_lacak}`;
  const pesan = STATUS_WA_TEMPLATE[t.status]
    .replace('{nama}', t.pelanggan?.nama || 'Pelanggan')
    .replace('{no_transaksi}', t.no_transaksi)
    .replace('{link_lacak}', linkLacak);
      ...
      <Modal open={!!detail} onClose={() => setDetail(null)} title={`Detail ${detail?.no_transaksi ?? ''}`}>
        {detail && (
          <div className="text-sm space-y-3">
            ...
            {buildWaLink(detail) ? (
              <a
                href={buildWaLink(detail)!}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 w-full mt-2 py-2.5 rounded-xl border-2 border-ink bg-[#25D366] text-white font-semibold shadow-doodle-sm hover:brightness-95"
              >
                Chat Pelanggan via WhatsApp
              </a>
            ) : (
              <p className="text-xs text-inkSoft italic">Nomor WhatsApp pelanggan tidak tersedia.</p>
            )}
          </div>
        )}
      </Modal>
